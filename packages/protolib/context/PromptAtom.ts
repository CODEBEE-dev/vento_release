...
export const PromptAtom = atom<PromptContext[]>([{
    ...
    generate: (prompt = '') => {
        ...
        The system is designed to extend the api (apps/vento) and to allow the user to extend the website (available at port 3000) or the mobile app (app/expo) by creating elements in packages/app, a package common for all apps.
        packages/app adds api endspoints to apps/vento by using a concept called bundles. A bundle is a folder with all screens, apis, objects, or other elements injectable in apps,centralized place to extend aspects of
        the apps without editing the app code, by using things imported from packages app.
        The user can also modify apps/vento or apps/adminpanel directly if specific customization is needed.
        ...
        The apps/vento service provides both administrative apis (at /api/core/v1) and user apis (at /api/v1), plus the reverse proxy and the mqtt bus for notifications and messages between services.

        The default bundle for the user to edit is in packages/app/, and the bundle files are in packages/app/bundles, for example packages/app/bundles/apis.ts, imported by apps/vento.
        ...
        The mqtt server is based on aedes and running in apps/vento, providing a mqtt server for the stack.
        ...
        - packages/app/bundles/coreApis.tsx to add core api endpoints at apps/vento (with preffix /api/core/v1 in reverse proxy)
        ...
        - /workspace/services Allows to view resource usage for the running apps (like apps/vento). Its normal to see apps duplicated, but with the preffix -dev, since protofy starts some apps twice, one in development mode and the other in production mode (compiled and without HMR)
