...
// Read initial tint: first from settings, then from SiteConfig, then fallback to 'blue'
const getInitialTint = (): string => {
  // Try to read from window.ventoSettings (loaded via settings.js in _document.tsx)
  if (typeof window !== 'undefined' && (window as any).ventoSettings?.['theme.accent']) {
    return (window as any).ventoSettings['theme.accent']
  }
  // Fallback to SiteConfig
  return SiteConfig?.ui?.['defaultTint'] ?? 'blue'
}

const defaultTint = getInitialTint()
