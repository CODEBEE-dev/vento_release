...
interface DataViewProps {
    ...
    // Permission resource name override (when URL path doesn't match permission resource)
    // e.g., '/api/core/v1/accounts' uses 'users' permission, not 'accounts'
...
const DataViewInternal = forwardRef(({
    ...
    // Extract resource name from sourceUrl for permission checking
    // e.g., '/api/core/v1/boards' -> 'boards'
    // Use permissionResource prop to override when URL path doesn't match permission resource
    const resourceName = useMemo(() => {
        if (permissionResource) return permissionResource
        if (!sourceUrl) return ''
        const parts = sourceUrl.split('/').filter(Boolean)
        return parts[parts.length - 1] || ''
    }, [sourceUrl, permissionResource])

    // Check if user has permission to create/delete this resource
    const canCreate = useHasPermission(`${resourceName}.create`)
    const canDelete = useHasPermission(`${resourceName}.delete`)
    ...
    // Wrap deleteable to also check delete permission
    const permissionAwareDeleteable = (item) => canDelete && deleteable(item)
    ...
    var defaultViews = [
        {
            ...
            props: {
                ...
                deleteable: permissionAwareDeleteable,
    ...
    return (<AsyncView atom={currentItems}>
        <YStack ref={ref} height="100%" f={1} p={contentPadding ?? "$6"}>
                    ...
                    <XStack pt="$3" px="$3" mb="$1" flexWrap='wrap'>
                        ...
                        <XStack ai="center" ml="$2" jc="flex-end" flexWrap='wrap' maxWidth={"100%"}>
                            ...
                            <XStack ai="center" flexWrap='wrap' marginLeft="$3" mb={"$1"} $xs={{ display: 'none' }}>
                                ...
                                {!hideDeleteAll && canDelete && <Tinted>
                                    <DataViewActionButton
                                        id="admin-dataview-add-btn"
                                        icon={Trash}
