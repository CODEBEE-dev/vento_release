...
export const ActionCardSettings = ({ board, actions, states, card, icons, onEdit = (data) => { }, onSave = () => { }, onClose = () => { }, errors, mode = "edit", tab = "rules", readOnly = false }) => {
  ...
  const allTabs = [
    ...
    }] : []),
    {
      ...
      content: (
        <TabContainer px="0px" py="0px">
          <DisplayEditor style={{ width: "100%", height: "fit-content" }} board={board} icons={icons} card={card} cardData={cardData} setCardData={setCardData} readOnly={readOnly} />
        </TabContainer>
      )
    },
    {
      ...
      content: (
        <TabContainer px="$4" py="$4">
          <ViewEditor cardData={cardData} setHTMLCode={setHTMLCode} readOnly={readOnly} />
        </TabContainer>
      )
    },
    {
      ...
      label: 'Raw',
      icon: <Braces size={"$1"} />,
      content: <SettingsEditor cardData={cardData} setCardData={setCardData} resolvedTheme={resolvedTheme} readOnly={readOnly} />
    },
  ]
  ...
  return (
    <YStack f={1} className={readOnly ? 'card-settings-readonly' : undefined}>
      {readOnly && (
        <style>{`
      ...
      <Tinted>
        <YStack f={1}>
            ...
            {!isCreateMode && (
              <XStack ai="center" gap="$2" zIndex={1}>
                ...
                <XStack ai="center" gap="$2" p="$1">
                  <XStack
                    cursor={!readOnly && hasChanges && !isSaving ? "pointer" : "default"}
                    padding="$2"
                    borderRadius="$2"
                    width="32px"
                    height="32px"
                    hoverStyle={!readOnly && hasChanges && !isSaving ? { backgroundColor: "$gray4" } : {}}
                    onPress={!readOnly && hasChanges && !isSaving ? handleSave : undefined}
                    pressStyle={{ opacity: 0.8 }}
                  >
                    {isSaving ? (
                      <Spinner size="small" color="var(--color8)" />
                    ) : (
                      <Save size={18} color={!readOnly && hasChanges ? "var(--color8)" : "var(--gray7)"} />
                    )}
                  </XStack>
                </XStack>
              </XStack>
          ...
          <Tinted>
            {allTabs.map((tabItem) => (
              <YStack className="card-settings-content" display={tabItem.id === selectedTab ? "flex" : "none"} key={tabItem.id} f={1} gap="$4">
                {tabItem.content || (
