...
export const ComponentCodeGeneration = ({ htmlCode, setHTMLCode }) => {
  ...
  return <YStack width={"100%"} height={"100%"} flex={1} borderRadius="$3">
    <YStack height={"100%"} flex={1} jc={loading ? "center" : "flex-start"} ai={loading ? "center" : "stretch"}>
      {
        loading && <PulseLoader
          color='#ececec'
        />
    ...
    {/* Área inferior: input siempre visible */}
    {!aiComponent && <YStack gap="$1" p="$1" position="relative">
      <TextArea
        w="100%"
