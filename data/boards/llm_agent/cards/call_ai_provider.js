...
// Get default provider and local model from settings
...
const sourceBoardParam = params.board
const sourceCardParam = params.card

// Params are already resolved by Vento (defaultValues point to board.current_request.params.*)
const promptMessage = params.message ?? ''
const promptConversation = params.conversation ?? []
const promptFiles = params.files ?? []
const promptImages = params.images ?? []
...
// Log if either the card setting OR the request param enables it
const shouldLog = params.log_ai_calls || board?.["current_request"]?.["params"]?.["log_ai_calls"]
// Priority: request params > card params > settings default
const requestProvider = board?.["current_request"]?.["params"]?.["provider"]
const requestModel = board?.["current_request"]?.["params"]?.["model"]

// Use request provider if exists and not 'default', else card params, else settings default
...
// Use request model if exists and not 'default', else card params, else provider's default
...
// Log AFTER reply - fire and forget (no await) to avoid blocking
if (shouldLog) {
  const sourceBoard = sourceBoardParam || 'llm_agent'
  const sourceCard = sourceCardParam || 'call_ai_provider'
  executeAction({
    name: "log_ai_call",
    params: {
      board: sourceBoard,
  ...
  }).catch(err => logger.error(`Error logging AI call: ${err}`))
}
