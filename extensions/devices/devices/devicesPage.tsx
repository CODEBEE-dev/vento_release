...
export default {
  component: ({ pageState, initialItems, itemData, pageSession, extraData }: any) => {
    ...
    const canUpdateDefinition = useHasPermission('deviceDefinitions.update')
    const canCreateDefinition = useHasPermission('deviceDefinitions.create')
    const canUpdateFiles = useHasPermission('files.update')
    ...
    const extraMenuActions = [
      {
        ...
        isVisible: (element) => canUpdateDevice
      },
      {
        text: "Edit template",
        icon: BookOpen,
        action: (element) => templateEditor.openDefinition(element.data.deviceDefinition),
        isVisible: (element) => Boolean(element.data.deviceDefinition) && canUpdateDefinition
      },
      {
        text: "Edit config",
        icon: Pencil,
        action: (element) => templateEditor.openDevice(element.data.name),
        isVisible: (element) => !element.data.deviceDefinition && element.getConfigFile() && canUpdateDevice
      },
      {
        text: "Upload definition file",
        icon: UploadCloud,
        action: (element) => { flashDevice(element) },
        isVisible: (element) => element.data.deviceDefinition && canUpdateDevice
      },
      {
        text: "Edit config file",
        icon: Pencil,
        action: (element) => { replace('editFile', element.getConfigFile()) },
        isVisible: (element) => element.getConfigFile() && canUpdateFiles
      },
      {
        ...
        isVisible: (element) => element.getConfigFile() && canUpdateDevice
      },
      {
        text: "View logs",
        icon: Bug,
        ...
        isVisible: (element) => element.getLogs() // Read-only, no permission needed beyond device read
      },
      {
        ...
        isVisible: (element) => element?.data?.data?.lastCompile?.success // Read-only, no permission needed beyond device read
      },
      {
      ...
      },
      {
        ...
        isVisible: (element) => element.data?.platform === "esphome" && element.getConfigFile() && canCreateDefinition
      }
    ]
