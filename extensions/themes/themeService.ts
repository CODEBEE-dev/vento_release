...
export default class ThemeService {
    ...
    static async selectTheme({themeId, token, accent}) {
        ...
        let res2
        res2 = await API.post(`/api/core/v1/settings/theme.accent?token=${token}`, { name: "theme.accent", value: accent ?? defaultTint })
        if(res2.isError) {
            res2 = await API.post(`/api/core/v1/settings?token=${token}`, { name: "theme.accent", value: accent ?? defaultTint })
        }
        return res
    }
}