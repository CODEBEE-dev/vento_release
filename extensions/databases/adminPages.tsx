...
export default {
    'databases': {
        component: ({ workspace, pageState, initialItems, pageSession }: any) => {
            ...
            const canBackup = useHasPermission('databases.backup')
    ...
    'databases/view': {
        component: ({ workspace, pageState, sourceUrl, initialItems, pageSession, extraData }: any) => {
            const canUpdate = useHasPermission('databases.update')
            const canDelete = useHasPermission('databases.delete')
                        ...
                        getCard: (data, width) => {
                            ...
                            return <YStack px={"$3"} pb="$4" f={1}>
                                <DataCard
                                    ...
                                    key={renew}
                                    onSave={canUpdate ? (content) => onSave(content, _key) : undefined}
                                    json={element}
                                    name={_key}
