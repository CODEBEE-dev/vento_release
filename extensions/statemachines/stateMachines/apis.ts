...
export default (app, context) => {
  ...
  const autoAPI = AutoAPI({
    ...
    permissions: {
      list: 'stateMachines.read',
      read: 'stateMachines.read',
      create: 'stateMachines.create',
      update: 'stateMachines.update',
      delete: 'stateMachines.delete'
    }
  })
  autoAPI(app, context)

  // start and stop instance
  app.get("/api/v1/statemachines/:instanceName/start", checkMachineInstanceName, handler(async (req, res, session) => {
    hasPermission(session, 'stateMachines.execute')
  ...
  app.get("/api/v1/statemachines/:instanceName/stop", checkMachineInstanceName, handler(async (req, res, session) => {
    hasPermission(session, 'stateMachines.execute')
    const instanceName = req.params.instanceName
    try {
  ...
  app.post("/api/v1/statemachines/:instanceName/emit", checkMachineInstanceName, handler(async (req, res, session) => {
    hasPermission(session, 'stateMachines.execute')
    const instanceName = req.params.instanceName
  ...
  // debugging options
  app.get("/api/v1/statemachines/:instanceName", checkMachineInstanceName, handler(async (req, res, session) => {
    hasPermission(session, 'stateMachines.read')
    const instanceName = req.params.instanceName
  ...
  app.post("/api/v1/statemachines", handler(async (req, res, session) => {
    hasPermission(session, 'stateMachines.create')
