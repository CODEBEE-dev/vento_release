...
import { handler, requirePermission } from 'protonode';
...
export default async (app: any, context: any) => {
     ...
     */
    app.get('/api/core/v1/matrix/status', requirePermission('chatbots.read'), handler(async (req: any, res: any) => {
        res.json({
            type: 'appservice',
     ...
     * Force sync agents
     */
    app.post('/api/core/v1/matrix/sync', requirePermission('chatbots.update'), handler(async (req: any, res: any) => {
        await syncAgents();
        res.json({
            success: true,
            agents: getAgents(),
     ...
     * List agents
     */
    app.get('/api/core/v1/matrix/agents', requirePermission('chatbots.read'), handler(async (req: any, res: any) => {
        res.json({
            agents: getAgents(),
        });
    }));
