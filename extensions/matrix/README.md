...
## CRITICAL: Bridged vs Native Users

**When sending DMs programmatically, ALWAYS use the bridged user ID (`@_vento_*`).**

| User Type | Format | Auto-accepts invites? |
|-----------|--------|----------------------|
| **Bridged** | `@_vento_{name}:vento.local` | **YES** (appservice handles it) |
| Native | `@{name}:vento.local` | NO (requires manual accept) |

**Why this matters:**
- The Mautrix appservice automatically accepts room invitations for bridged users
- Native Matrix users don't auto-accept, so programmatically created DM rooms fail
- The bot never joins the room, never sees messages, never responds

**Common mistake:**
```typescript
// WRONG - native user won't auto-accept the room invitation
sendDM('@vento_agent:vento.local', 'Hello!')

// CORRECT - bridged user auto-accepts via appservice
sendDM('@_vento_vento_agent:vento.local', 'Hello!')
```

This is why DMs created via Cinny's UI work but programmatic ones fail - users click on the bridged user (`@_vento_*`) in the UI.
