...
export const getExecuteAction = (actions, board, states, contextId?: string) => `
const actions = ${JSON.stringify(actions)}
const __ephemeralContextId = ${contextId ? `'${contextId}'` : '_contextId'};
async function execute_action(url_or_name, params={}) {
    console.log('Executing action from getExecuteAction.ts:', url_or_name, params);

    // Check for cross-board action syntax: "board/action"
    if (url_or_name.includes('/') && !url_or_name.startsWith('/') && !url_or_name.startsWith('http')) {
        const [targetBoard, actionName] = url_or_name.split('/');
        console.log('Cross-board action: board=' + targetBoard + ', action=' + actionName);

        // Propagate stackTrace and contextId
        params._stackTrace = JSON.stringify(stackTrace);
        if (__ephemeralContextId) {
            params._contextId = __ephemeralContextId;
        }

        const url = '/api/core/v1/boards/' + encodeURIComponent(targetBoard) + '/actions/' + encodeURIComponent(actionName) + '?token=${getServiceToken()}';
        const response = await API.post(url, params);
        if (response.isError) {
            throw new Error(JSON.stringify(response.error || 'Error executing cross-board action'));
        }
