...
const getActionBar = (generateEvent) => {
  ...
  const { isJSONView, autopilot, setViewMode, viewMode, tabVisible, canWrite } = useBoardControls();
  ...
  const undoRedoButtons = toggleUndoRedoButtons ? [<ActionBarButton
    ...
    Icon={Undo}
    disabled={!canWrite || !boardId || !canUndo}
    onPress={async () => {
      try {
        await undo?.();
  ...
  <ActionBarButton
    ...
    Icon={Redo}
    disabled={!canWrite || !boardId || !canRedo}
    onPress={async () => {
      try {
        await redo?.();
      } catch (e) { console.error(e); }
  ...
  const bars = {
    ...
    'BoardView': [
      <ActionBarButton tooltipText={canWrite ? "Add" : "Read-only board"} Icon={Plus} disabled={!canWrite} onPress={() => generateEvent({ type: "open-add" })} />,
      ...undoRedoButtons,
      ...historyVersion,
      ...
      <ActionBarButton tooltipText={canWrite ? "Relayout" : "Read-only board"} Icon={LayoutGrid} disabled={!canWrite} onPress={() => generateEvent({ type: "relayout" })} />,
      ...
      <ActionBarButton tooltipText={canWrite ? "Agent Settings" : "Read-only settings"} selected={tabVisible == "board-settings"} Icon={Settings} onPress={() => generateEvent({ type: "board-settings" })} />,
