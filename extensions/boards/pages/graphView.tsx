...
const materializeNodes = (
    ...
    return cards.map(c => {
        ...
        return {
            ...
            position: { x, y },
            data: { ...c, ports: buildPortsFor(c.name, edges, c.type), readOnly },
...
const materializeLayer = (
    ...
    if (isFlat) {
        return {
            contentNodes: materializeNodes(cards, positions, sizes, edges, savedLayout, layerName, undefined, { x: 0, y: yOffset }, readOnly),
    ...
    return {
        groupNode,
        contentNodes: materializeNodes(cards, positions, sizes, edges, savedLayout, layerName, groupNode.id, { x: minX, y: minY }, readOnly),
...
const Flow = memo(({
    ...
    const handleConnect = useCallback((connection: Connection) => {
        if (!onCreateEdge || readOnly) return;
    ...
    }, [edges, setEdges, onCreateEdge, readOnly]);
...
export const GraphView = memo(({
    ...
    const { initialNodes, initialEdges } = useMemo(() => {
        ...
        for (const [layerName, layerCards] of Array.from(grouped.entries())) {
            const { groupNode, contentNodes: layerNodes, yIncrement } = materializeLayer(layerName, layerCards, edges, yOffset, layout, readOnly);
    ...
    }, [cards, layout, linksKey, readOnly]);
