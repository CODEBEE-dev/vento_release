...
export default (app, context) => {
  app.get("/api/v1/wled/state", handler(async (req, res, session) => {
    hasPermission(session, 'wled.read')
    const { address } = req.params;
    const state = await API.get(`http://${address}/json/state`)
    res.send(state);
  }));

  app.post("/api/v1/wled/action/:address", handler(async (req, res, session) => {
    hasPermission(session, 'wled.execute')
    let result
    const { address } = req.params;
