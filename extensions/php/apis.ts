...
export default (app, context) => {
    ...
    // execute some php
    app.get("/api/v1/php", getPhpFiles, handler(async (req, res, session) => {
        hasPermission(session, 'php.execute')
    ...
    app.get("/api/v1/phpRaw", getPhpFiles, handler(async (req, res, session) => {
        hasPermission(session, 'php.execute')
        const phpFiles = req.phpFiles
        const { file } = req.query;
        if (!file) {
