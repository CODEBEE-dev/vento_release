...
const SaveButton = ({
  ...
  const canUpdate = useHasPermission('files.update')
  ...
  // Don't render if user doesn't have permission to update files
  ...
  useEventEffect(onEvent, { path: 'services/vento-dev/start' })
  useEventEffect(onEventCrash, { path: 'services/vento-dev/crash' })
...
export const CodeView = ({
  ...
  const monaco = useMemo(() => {
    const effectiveMonacoProps = {
      ...monacoProps,
      disabled: readOnly || monacoProps?.disabled,
    };
    return monacoInstance ?? <Monaco
      ...
      onChange={readOnly ? undefined : (code) => { onCodeChange(code); sourceCode.current = code }}
      {...effectiveMonacoProps}
    />
  }, [sourceCode.current, path, resolvedTheme, readOnly, monacoProps])
  ...
  const getFlows = () => {
    const flowContent = <Flows
      ...
      onEdit={readOnly ? undefined : (code) => { onFlowChange(code); sourceCode.current = code }}
      setIsModified={setIsModified}
      ...
      {...flowsProps}
      />
    if (!readOnly) return flowContent;
    return <YStack pointerEvents="none" opacity={0.85}>{flowContent}</YStack>;
  }
