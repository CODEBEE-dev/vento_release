...
import { checkPermission } from 'protobase';
import fs from "fs";
...
const isAuthorized = (req, session) => {
    ...
    return checkPermission(session, 'vision.execute')
}

export default async (app: Application, context: typeof APIContext) => {

    app.get('/api/core/v1/vision/detect', handler(async (req, res, session) => {
    ...
    app.post('/api/core/v1/vision/frame/set', handler(async (req, res, session) => {
        if (!isAuthorized(req, session)) {
            return res.status(401).send({ error: "Unauthorized" });
        }
    ...
    app.get('/api/core/v1/vision/frame/get', handler(async (req, res, session) => {
        if (!isAuthorized(req, session)) {
            return res.status(401).send({ error: "Unauthorized" });
        }
    ...
    app.get('/api/core/v1/vision/describe', handler(async (req, res, session) => {
        if (!isAuthorized(req, session)) {
            return res.status(401).send({ error: "Unauthorized" });
        }
