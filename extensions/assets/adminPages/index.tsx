...
function FilesPage({ initialFilesState, pageSession, initialItems, pageState }: any) {
  ...
  const canInstall = useHasPermission('assets.update')
        ...
        <DataView
          ...
          columns={DataTable2.columns(
            ...
            DataTable2.column("", row =>
              canInstall ? <InstallButton element={row} loading={loading} br="$10" size="$3" w="80px" onPress={() => onInstallAsset(row.name)} /> : null
            ...
            getCard: (element: any, width: any) => {
              return <YStack br="$6" w={width} bc="$bgPanel" overflow='hidden'>
                ...
                <YStack p="$4" gap="$4" ai="flex-start">
                  ...
                  {canInstall && <InstallButton element={element} loading={loading} onPress={() => onInstallAsset(element.name)} />}
                </YStack>
              </YStack>
            }
          }}
          onAddButton={canCreate ? () => setExplorerVisible(true) : undefined}
          model={AssetsModel}
          pageState={pageState}
          icons={{}}
        />
