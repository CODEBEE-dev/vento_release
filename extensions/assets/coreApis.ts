import { handler, getServiceToken, AutoAPI, getRoot, requirePermission } from "protonode";
import { API, getLogger, hasPermission } from "protobase";
...
export default (app, context) => {
    ...
    const AssetsAutoAPI = AutoAPI({
        ...
        permissions: {
            list: 'assets.read',
            read: 'assets.read',
            create: 'assets.install',
            update: 'assets.install',
            delete: 'assets.install'
        },
    ...
    app.get('/api/core/v1/assets/install/all', handler(async (req, res, session) => {
        hasPermission(session, 'assets.install')
    ...
    app.get('/api/core/v1/assets/:id/config', requirePermission('assets.read'), async (req, res) => {
    ...
    app.post('/api/core/v1/assets/:id/config', requirePermission('assets.install'), async (req, res) => {
        const assetId = req.params.id;
        const rootPath = getRoot(req);
        const assetPath = dataDir(rootPath, assetId);
