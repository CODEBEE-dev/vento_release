...
import { z, Schema, BaseSchema, AutoModel, ProtoModel, hasPermission } from 'protobase';
import { AutoActions, AutoAPI, handler } from 'protonode'
...
export default (app, context) => {
    ...
    app.get('/api/v1/objects/reload', handler(async (req, res, session) => {
        hasPermission(session, 'objects.update')
        reloadObjectAPIs(app, context)
        res.json({ status: 'ok' })
    }))
}