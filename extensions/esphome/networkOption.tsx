...
const DevicesWizard = ({ onCreated, onBack }: { onCreated: (data?: any) => void, onBack?: () => void }) => {
    ...
    const buildComponents = (boardInfo: any, wifi: WifiNetwork, mqttUrl?: string | null) => {
        ...
        const pickIndexExcluding = (list: number[], excludeList: (number | null)[]) => {
            for (const idx of list) {
                if (!excludeList.includes(idx)) return idx
            }
            return null
        }
        ...
        const mqttIdx = mqttComponent
            ...
            : null
        const otaIdx = pickIndexExcluding(virtualIndexes, [wifiIdx, mqttIdx])
            ?? pickIndexExcluding(ioIndexes, [wifiIdx, mqttIdx])
            ?? null

        const totalPorts = Math.max(ports.length, (wifiIdx !== null ? wifiIdx + 1 : 0), (mqttIdx !== null ? mqttIdx + 1 : 0), (otaIdx !== null ? otaIdx + 1 : 0), 2)
