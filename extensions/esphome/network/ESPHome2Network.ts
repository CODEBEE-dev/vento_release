...
const dumpYaml = (schematic: any) => {
    ...
    const binarySensors = components
        .filter((c: any) => c.category === "binary_sensor")
        .map((component: any) => {
            const raw = cloneOrEmpty(component.meta?.raw);
            raw.platform = raw.platform || "gpio";
            raw.id = component.id;
            raw.name = component.label || component.id;
            const inputPin = findPin(component, "input");
            if (inputPin) {
                const parsedPin = toNumberFromGpio(inputPin.connectedTo);
                if (parsedPin !== undefined) {
                    const inverted = component.editableProps?.inverted?.default;
                    const pullup = component.editableProps?.pullup?.default;
                    if (inverted || pullup) {
                        raw.pin = {
                            number: parsedPin,
    ...
    if (binarySensors.length) {
        baseConfig.binary_sensor = normalizeSection(binarySensors);
    } else {
        delete baseConfig.binary_sensor;
    }
