...
import {
  buildBinarySensorComponents,
  buildBinarySensorSubsystems,
  buildBinarySensorTemplate,
} from './binarySensor'
...
const builders: ComponentBuilder[] = [
  ...
  },
  {
    key: 'binary_sensor',
    build: (config, _context, templateContext) => ({
      components: buildBinarySensorComponents(config),
      subsystems: buildBinarySensorSubsystems(config),
      template: templateContext ? buildBinarySensorTemplate(templateContext) : undefined,
    }),
  },
]
