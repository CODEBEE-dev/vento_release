...
import { API, generateEvent, hasPermission } from 'protobase'
...
export default (app, context) => {
    ...
    app.get('/api/v1/arduinos/connectedDevices', handler(async (req, res, session) => {
        hasPermission(session, 'arduino.read')
        const connectedDevices = serialManager.getConnectedDevices()
        return res.status(200).json({ connectedDevices })
    }))

    app.get('/api/v1/arduinos/:name/disconnect', handler(async (req, res, session) => {
        hasPermission(session, 'arduino.execute')
    ...
    app.get('/api/v1/arduinos/:name/connect', handler(async (req, res, session) => {
        hasPermission(session, 'arduino.execute')
        const name = req.params.name
        const arduino = ArduinosModel.load({ name })
        if (!arduino) {
    ...
    app.get('/api/v1/arduinos/:name/transport/send/:data', handler(async (req, res, session) => {
        hasPermission(session, 'arduino.execute')
        const name = req.params.name
    ...
    app.get('/api/v1/transports/serial/ports/available', handler(async (req, res, session) => {
        hasPermission(session, 'arduino.read')
    ...
    app.post('/api/v1/arduinos/:name/genericPrompt', handler(async (req, res, session) => {
        hasPermission(session, 'arduino.execute')
    ...
    app.post('/api/v1/arduinos/:name/generateCode', handler(async (req, res, session) => {
        hasPermission(session, 'arduino.execute')
    ...
    app.get('/api/v1/transports/serial/arduino', handler(async (req, res, session) => {
        hasPermission(session, 'arduino.read')
