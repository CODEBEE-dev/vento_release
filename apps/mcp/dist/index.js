...
function buildResources(boards) {
    const resources = [];
    for (const board of boards) {
...
async function executeTool(name, args) {
    ...
    try {
        if (actionName === 'chat') {
            ...
            const res = await fetch(`${API_BASE}/api/agents/v1/${boardName}/agent_input?token=${token}`, {
                method: 'POST',
            ...
            // Execute action
            const res = await fetch(`${API_BASE}/api/core/v1/boards/${boardName}/actions/${actionName}?token=${token}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(args)
            });
...
async function readResource(uri) {
    ...
    try {
        const res = await fetch(`${API_BASE}/api/core/v1/boards/${boardName}/cards/${cardName}?token=${token}`);
        if (!res.ok) {
            return JSON.stringify({
...
async function main() {
    ...
    // Create MCP server
    const server = new Server({ name: "vento", version: "1.0.0" }, { capabilities: { tools: {}, resources: {} } });
    // Handler: list tools
