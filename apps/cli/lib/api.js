...
/**
 * Get all base cards (card definitions)
 */
async function getCards() {
  const data = await apiFetch('/api/core/v1/cards?itemsPerPage=10000');
  return data?.items || [];
}

/**
 * Get board automation code
 */
async function getAutomation(boardName) {
  return await apiFetch(`/api/core/v1/boards/${boardName}/automation`);
}

/**
 * Set board automation code
 */
async function setAutomation(boardName, code) {
  return await apiFetch(`/api/core/v1/boards/${boardName}/automation`, {
    method: 'POST',
 ...
 */
async function startAutopilot(boardName) {
  return await apiFetch(`/api/core/v1/boards/${boardName}/autopilot/on`);
}

/**
 * Stop autopilot for a board
 */
async function stopAutopilot(boardName) {
  return await apiFetch(`/api/core/v1/boards/${boardName}/autopilot/off`);
}
