...
/**
 * List all base cards (card definitions)
 */
async function listCards(groupFilter) {
  const spin = ui.spinner('Loading cards...');
  ...
  try {
    ...
    if (groupFilter) {
      cards = cards.filter(c => c.group === groupFilter);
    }

    ui.header('Vento Base Cards', ui.emoji.tool);

    if (cards.length === 0) {
      ui.warning(groupFilter ? `No cards found for group "${groupFilter}"` : 'No cards found');
      return;
    }

    // Group cards by group.tag for better organization
    const grouped = {};
    for (const card of cards) {
      const key = `${card.group}.${card.tag}`;
      if (!grouped[key]) grouped[key] = [];
      grouped[key].push(card);
    }

    for (const [groupTag, groupCards] of Object.entries(grouped)) {
      console.log('\n' + ui.theme.agent(`── ${groupTag} ──`));

      const rows = groupCards.map(c => {
        // Truncate rulesCode for display
        let codePreview = '-';
        if (c.defaults?.rulesCode) {
          codePreview = c.defaults.rulesCode
            .replace(/\n/g, ' ')
            .substring(0, 60);
          if (c.defaults.rulesCode.length > 60) codePreview += '...';
        }

        return [
          ui.theme.tool(c.name || c.id),
          ui.theme.muted(c.defaults?.description || c.templateName || '-'),
          ui.theme.muted(codePreview)
        ];
      });

      ui.table(['Name', 'Description', 'Code Preview'], rows);
    }

    ui.blank();
    ui.info(`${cards.length} card(s) available`);

  } catch (err) {
    spin.stop();
    ui.error(`Failed to load cards: ${err.message}`);
  }
}
